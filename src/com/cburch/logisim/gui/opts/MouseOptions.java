...
class MouseOptions extends OptionsPanel {
        ...
        public void valueChanged(ListSelectionEvent e) {
            ...
            if(row < 0) {
                ...
                attr_table.setAttributeSet(tool.getAttributeSet(),
                    ToolActions.createTableListener(getProject(), tool));
            }
        }
    ...
    private class MappingsModel extends AbstractTableModel {
        ...
        public Object getValueAt(int row, int column) {
            Integer key = (Integer) cur_keys.get(row);
            if(column == 0) {
                return InputEventUtil.toDisplayString(key.intValue());
            } else {
                Tool tool = (Tool) getOptions().getMouseMappings().getToolFor(key);
        ...
        Integer getKey(int row) {
            return (Integer) cur_keys.get(row);
        }

        Tool getTool(int row) {
            if(row < 0 || row >= cur_keys.size()) return null;
            Integer key = (Integer) cur_keys.get(row);
            return (Tool) getOptions().getMouseMappings().getToolFor(key.intValue());
        }
    ...
    public MouseOptions(OptionsFrame window) {
        ...
        attr_table = new AttributeTable(getOptionsFrame(), getProject());
